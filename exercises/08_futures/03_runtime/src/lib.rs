...
async fn _fr<T>(listener: TcpListener, reply: T)
where
    T: Display + Send + Sync + 'static,
{
    loop {
        let (mut s, _addr) = listener.accept().await.unwrap();
        let (_rx, mut tx) = s.split();
        tx.write_all(format!("{}", reply).as_bytes()).await.unwrap();
    }
}
...
{
    let r = Arc::new(reply);
    let h1 = tokio::spawn(_fr(first, r.clone()));
    let h2 = tokio::spawn(_fr(second, r));
